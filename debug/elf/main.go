...
func printHeader(fh *elf.FileHeader) {
	...
	log.Printf("fh.Type: %s", fh.ByteOrder)
	log.Printf("fh.Machine: %s", fh.Machine)
}

func printSection(s *elf.Section) {
	log.Printf(`section [Type: %s, Flags: %s, Addr: %x, Offset: %v, Size: %v, 
		Link: %v, Info: %v, Addralign: %v, Ensize: %v]`, s.Type, s.Flags,
		s.Addr, s.Offset, s.Size, s.Link, s.Info, s.Addralign, s.Entsize)
}

func printProgramHeader(p *elf.Prog) {
	log.Printf(`program header [Type: %s, Flags: %s, Off: %#x, Vaddr: %#x, 
		Filesz: %#x, Memsz: %#x, Align: %#x]`, p.Type, p.Flags, p.Off, p.Vaddr,
